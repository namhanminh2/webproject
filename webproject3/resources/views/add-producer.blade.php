...
<html lang="en">
  ...
  <body>
    ...
    <div class="container" style="margin-top:20px">
        <div class ="row">
            <div class="col-md-12">
                <h2> Add PC Producer </h2>
                @if(Session::has('success'))
                <div class = "alert alert-success" role = "alert"> 
                    {{Session::get('success')}}
                </div>
                @endif
                <form method="post" action="{{url('save-producer')}}">
                    @csrf
                    <div class="md -3">
                        <label class="form-lebel">PC Producer</label>
                        <input type="text" class="form-control" name="producer" placeholder="Enter PC Producer">
                    </div>
                    @error('producer')
                    ...
                    @enderror         
                    <div class="md -3">
                        <label class="form-lebel">Address Producer</label>
                        <input type="text" class="form-control" name="address1" placeholder="Enter Address Producer">
                    </div>
                    @error('producer')
                        <div class = "alert alert-danger" role = "alert"> 
                            {{$message}}
